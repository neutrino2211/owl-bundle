  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (i % 2 === 1) {
      ...
      if (typeof innerTree.rawNodeName === 'string' && isFragment) {
        childNodes.push.apply(childNodes, toConsumableArray(innerTree.childNodes));
      } else {
 ...
 * @return {Object} vTree
 */
var HTMLElement = function HTMLElement(nodeName, rawAttrs, supplemental, options) {
  var match = null;
      ...
      for (var i = 0; i < parts.length; i++) {
        ...
        if (i % 2 === 1) {
          if (attributes[newName]) {
            ...
            var isObject = (typeof newName === 'undefined' ? 'undefined' : _typeof(newName)) === 'object';

            if (isObject && !isArray$1(newName) && newName) {
              assign$1(attributes, newName);
            ...
            } else if (newName && (typeof newName === 'undefined' ? 'undefined' : _typeof(newName)) !== 'object') {
              attributes[newName] = supplemental.attributes[_value];
            }
          }
        }
...
function parse(html, supplemental) {
      ...
      if (options.strict || blockText.has(match[2])) {
        ...
        if (process$1.env.NODE_ENV !== 'production') {
          if (index === -1 && options.strict) {
            ...
            var caret = Array(spaceEx.exec(_markup[0]).index + closeMarkup.length - 1).join(' ') + '^';

            var name = supplemental ? supplemental.tags[0].name : match[2];

            // Craft the warning message and inject it into the markup.
            _markup.splice(1, 0, caret + '\n    Possibly invalid markup. <' + name + '> is not a self closing tag.\n            ');
    ...
    if (match[1] || match[4] || selfClosing.has(match[2])) {
      if (process$1.env.NODE_ENV !== 'production') {
        if (currentParent && match[2] !== currentParent.rawNodeName && options.strict) {
          var _nodeName = currentParent.rawNodeName;

          // Find a subset of the markup passed in to validate.
          var _markup2 = html.slice(tagEx.lastIndex - match[0].length).split('\n').slice(0, 3);

          // Position the caret next to the first non-whitespace character.
          var _caret = Array(spaceEx.exec(_markup2[0]).index).join(' ') + '^';

          // Craft the warning message and inject it into the markup.
          _markup2.splice(1, 0, _caret + '\n  Possibly invalid markup. Saw ' + match[2] + ', expected ' + _nodeName + '...\n          ');
  ...
  if (process$1.env.NODE_ENV !== 'production') {
    if ((remainingText.includes('>') || remainingText.includes('<')) && options.strict) {
      var _nodeName2 = currentParent.rawNodeName;

      // Find a subset of the markup passed in to validate.
      var _markup3 = [remainingText];

      // Position the caret next to the first non-whitespace character.
      var _caret2 = Array(spaceEx.exec(_markup3[0]).index).join(' ') + '^';

      // Craft the warning message and inject it into the markup.
      if (remainingText.includes('>')) {
        _markup3.splice(1, 0, _caret2 + '\n  Possibly invalid markup. Opening tag was not properly opened.\n        ');
      } else {
        _markup3.splice(1, 0, _caret2 + '\n  Possibly invalid markup. Opening tag was not properly closed.\n        ');
      }
...
function handleTaggedTemplate(strings) {
  ...
  if (strings.length === 1 && !values.length) {
    ...
    var _parse = parse(strings[0], null, { strict: _strict }),
  ...
  var supplemental = { attributes: {}, children: {}, tags: {} };
  ...
  var _parse2 = parse(HTML, supplemental, { strict: strict }),
...
// just like HTML.
handleTaggedTemplate.strict = function () {
  handleTaggedTemplate.isStrict = true;
  return handleTaggedTemplate.apply(undefined, arguments);
};
